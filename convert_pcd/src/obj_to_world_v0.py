...
pub = rospy.Publisher('cube_world', PointCloud2, queue_size=1000)
...
def callback(cube, wheel, person):
    global x_neck,x_nose,x_ankle,y_neck,y_nose,y_ankle,z_neck,z_nose,z_ankle
    ...
    d = float(x_neck - x_nose);
    e = float(y_neck - y_nose);
    f = float(z_neck - z_nose);
    a = float(-(x_nose - x_ankle));
    b = float(-(y_nose - y_ankle));
    c = float(-(z_nose - z_ankle));
    ...
    if not length==0:
        new_pose = [-cube.pose.position.x*normal[0]/length*0.8+x_center, -(-cube.pose.position.y*normal[1]/length*0.8+y_center), -cube.pose.position.z*normal[2]*0.8/length+z_center]
        points = []
        colors = []
        ...
        for p in pc2.read_points(wheel, field_names = ("x", "y", "z"), skip_nans=True):
            point = np.array([p[0],p[1],p[2]])
            point = np.array(point)
            points.append(obj_rotation.dot(point.transpose()).transpose())
            colors.append([1.0,0.0,0.0])
        points = np.array(points)
        points[:,0] = (points[:,0]-points[:,0].mean())/10 + new_pose[0]
        points[:,1] = (points[:,1]-points[:,1].mean())/10 + new_pose[1]
        points[:,2] = (points[:,2]-points[:,2].mean())/10 + new_pose[2]      
        colors = np.array(colors)
        msg = xyzrgb_array_to_pointcloud2(points, colors, stamp=rospy.get_rostime(), frame_id='cube_points', seq=None)
        pub.publish(msg)
        br = tf.TransformBroadcaster()
        br.sendTransform((new_pose[0], new_pose[1], new_pose[2]),
                        tf.transformations.quaternion_from_euler(0, 0, 0),
                        rospy.Time.now(),
                        "cube_points",
                        "map")
    # else:
    #     new_pose = [cube.pose.position.x, cube.pose.position.y, cube.pose.position.z]
    #     points = []
    #     colors = []
    #     quaternion = Quaternion(array=np.array([cube.pose.orientation.x, cube.pose.orientation.y, cube.pose.orientation.z, cube.pose.orientation.w]))
    #     for p in pc2.read_points(wheel, field_names = ("x", "y", "z"), skip_nans=True):
    #         point = np.array([p[0],p[1],p[2]])
    #         print(point)
    #         points.append(quaternion.rotate(point))
    #         colors.append([1.0,0.0,0.0])
    #     points = np.array(points)
    #     points[:,0] = points[:,0]-points[:,0].mean() + new_pose[0]
    #     points[:,1] = points[:,1]-points[:,1].mean() + new_pose[1]
    #     points[:,2] = points[:,2]-points[:,2].mean() + new_pose[2]     
    #     colors = np.array(colors)
    #     msg = xyzrgb_array_to_pointcloud2(points, colors, stamp=rospy.get_rostime(), frame_id='cube_points', seq=None)
    #     pub.publish(msg)
    #     world_x = points[:,0].mean()
    #     world_y = points[:,1].mean()
    #     world_z = points[:,2].mean()
    #     br = tf.TransformBroadcaster()
...
def listener():
    ...
    ts = message_filters.ApproximateTimeSynchronizer([cube_sub, wheel_points_sub, human_sub], 1000, 0.1, allow_headerless=True)
